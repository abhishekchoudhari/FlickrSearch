...
class RACSignalEx {
    class func combineLatestAs<T, U, Q, R: AnyObject>(signals:[RACSignal], reduce:(T,U,Q) -> R) -> RACSignal {
        return RACSignal.combineLatest(signals).mapAs {
            (tuple: RACTuple) -> R in
            return reduce(tuple.first as! T, tuple.second as! U, tuple.third as! Q)
        }
    }
}
